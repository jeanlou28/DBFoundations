...
-- 2024-08-14, Jean Mondesir,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JeanMondesir')
	 Begin 
	  Alter Database [Assignment07DB_JeanMondesir] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JeanMondesir;
	 End
	Create Database Assignment07DB_JeanMondesir;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JeanMondesir;
...
Select 
    ...
    FORMAT(i.InventoryDate, 'MMMM, yyyy') as InventoryDate, 
    i.Count as InventoryCount,
    ISNULL(LAG(i.Count, 1) OVER (Partition By p.ProductName Order By i.InventoryDate), 0) as PreviousMonthCount
From 
    dbo.Products p
Join 
    dbo.Inventories i On p.ProductID = i.ProductID
